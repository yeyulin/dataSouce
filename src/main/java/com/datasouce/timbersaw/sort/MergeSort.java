...
public class MergeSort {
    ...
    public void mergeSort5(int arr[]) {
        mergeSort5(arr, 0, arr.length - 1);
    }

    private void mergeSort5(int[] arr, int left, int right) {
        if (left >= right) {
            return;
        }
        int mid = left / 2 + right / 2;
        ...
        if (arr[mid] > arr[mid + 1]) {
            mergeSort5(arr, left, mid, right);
        }
    }

    private void mergeSort5(int[] arr, int left, int mid, int right) {
        int[] aux = Arrays.copyOfRange(arr, left, right + 1);
        int li = left;
        int ri = mid + 1;
        for (int i = left; i <= right; i++) {
